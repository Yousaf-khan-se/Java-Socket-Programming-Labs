...
public class I228797_F_Q4_Sender2 {
	public static void main(String[] args) throws IOException {
		...
		try (Scanner scan = new Scanner(System.in)) {
			while (!c_str.equals("e")) {
				...
				if (c_str.equals("g")) {
					DatagramPacket data_packet = new DatagramPacket(c_str.getBytes(), c_str.length(),
							InetAddress.getLocalHost(), 2222);
					data_client.send(data_packet);
					data_client.receive(received_packet);
					System.out.println("The Server time is "
		...
		c_str = "e";
		DatagramPacket data_packet = new DatagramPacket(c_str.getBytes(), c_str.length(), InetAddress.getLocalHost(),
				2222);
